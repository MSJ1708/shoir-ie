...
def render_export_bar(module: str, tables: Sequence[Tuple[str,pd.DataFrame]], figures: Sequence[Tuple[str,Any]]=(), tier: str="Starter", username: str="unknown"):
    ...
    auto_keys = {
        "Quality Engineering & Reliability":["quality_spc_result","quality_limits_result","fmea_result"],
        "Industrial Simulation Lab":["sim_des_result","sim_agent_result","sd"],
        "3D Factory Designer":["factory3d_dist_result"],
        "Multi-Objective Optimization":["multiobj_result","pareto"],
        "Robust & Resilient Optimization":["robust_result"],
        "Experiment Lab":["experiment_results"],
        "Capital Investment & Engineering Economics":["capex_result"],
        "Workforce Engineering":["balance_result","balance_metrics"],
        "Industrial Sustainability & LCA":["sustain_result"],
        "Benchmarking & Engineering Standards":["benchmark_result"],
        "Advanced ML Demand Forecasting":["forecast_result"],
    }
    combined=list(tables)
    labels={str(x[0]) for x in combined}
    for key in auto_keys.get(module,[]):
        value=st.session_state.get(key)
        if isinstance(value,pd.DataFrame) and key not in labels:
            combined.append((key,value))
        elif isinstance(value,dict) and value and key not in labels:
            combined.append((key,pd.DataFrame([value])))

    st.markdown("---")
    st.subheader("📤 Results, Evidence & Downloads")
    st.caption("Export the complete engineering study with its generated tables and reproducibility manifest.")

    slug=re.sub(r"[^A-Za-z0-9]+","_",module).strip("_").lower()
    artifacts=[]
    for label,df in combined:
        if isinstance(df,pd.DataFrame):
            artifacts.append({
                "Artifact":str(label),"Rows":len(df),"Columns":len(df.columns),
                "SHA-256":hashlib.sha256(df.to_csv(index=False).encode("utf-8")).hexdigest(),
                "Generated By":username,"Generated At":_now(),
            })
    manifest_json=json.dumps({"module":module,"actor":username,"generated_at":_now(),"artifacts":artifacts},indent=2).encode("utf-8")

    csv_zip=io.BytesIO()
    with zipfile.ZipFile(csv_zip,"w",zipfile.ZIP_DEFLATED) as z:
        for label,df in combined:
            if isinstance(df,pd.DataFrame):
                safe=re.sub(r"[^A-Za-z0-9]+","_",str(label)).strip("_").lower() or "table"
                z.writestr(f"tables/{safe}.csv",df.to_csv(index=False))
        z.writestr("manifest.json",manifest_json)
    csv_zip_bytes=csv_zip.getvalue()

    xlsx_bytes=pdf_bytes=pptx_bytes=None
    try:
        xlsx_bytes=build_excel_report("Shoir-IE | "+module,combined,figures)
    except Exception as exc:
        st.warning(f"Excel generation skipped safely: {exc}")
    if tier_allows(tier,"Enterprise"):
        try: pdf_bytes=export_pdf("Shoir-IE | "+module,combined,figures)
        except Exception as exc: st.warning(f"PDF generation skipped safely: {exc}")
        try: pptx_bytes=export_pptx("Shoir-IE | "+module,combined,figures)
        except Exception as exc: st.warning(f"PowerPoint generation skipped safely: {exc}")
    ...
    with zipfile.ZipFile(bundle,"w",zipfile.ZIP_DEFLATED) as z:
        ...
        for label,df in combined:
            if isinstance(df,pd.DataFrame):
                safe=re.sub(r"[^A-Za-z0-9]+","_",str(label)).strip("_").lower() or "table"
                z.writestr(f"tables/{safe}.csv",df.to_csv(index=False))
        if xlsx_bytes: z.writestr("shoir_ie_results.xlsx",xlsx_bytes)
        if pdf_bytes: z.writestr("shoir_ie_report.pdf",pdf_bytes)
        if pptx_bytes: z.writestr("shoir_ie_report.pptx",pptx_bytes)
    bundle_bytes=bundle.getvalue()

    b1,b2,b3,b4,b5=st.columns(5)
    with b1:
        if xlsx_bytes: st.download_button("📊 Excel",xlsx_bytes,f"shoir_ie_{slug}.xlsx","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",use_container_width=True)
    with b2:
        st.download_button("🗂️ CSV Bundle",csv_zip_bytes,f"shoir_ie_{slug}_csv.zip","application/zip",use_container_width=True)
    with b3:
        st.download_button("📋 Manifest",manifest_json,f"shoir_ie_{slug}_manifest.json","application/json",use_container_width=True)
    with b4:
        if pdf_bytes: st.download_button("📄 PDF",pdf_bytes,f"shoir_ie_{slug}.pdf","application/pdf",use_container_width=True)
        else: st.caption("PDF • Enterprise")
    with b5:
        st.download_button("📦 Complete Bundle",bundle_bytes,f"shoir_ie_{slug}_complete.zip","application/zip",use_container_width=True)
    if pptx_bytes:
        st.download_button("📽️ PowerPoint",pptx_bytes,f"shoir_ie_{slug}.pptx","application/vnd.openxmlformats-officedocument.presentationml.presentation",use_container_width=True)
    log_security_event(username,"report_export_manifest",module)
    ...
    if module=="Engineering Validation Center":
        ...
        with tabs[0]:
            ...
            if "quality_spc_result" in st.session_state:
                _quality_result = {**dict(st.session_state.get("quality_spc_result", {})), **dict(st.session_state.get("quality_limits_result", {}))}
                q1,q2,q3,q4 = st.columns(4)
                q1.metric("Cpk", f"{_quality_result.get('Cpk', float('nan')):.3f}")
                q2.metric("Cp", f"{_quality_result.get('Cp', float('nan')):.3f}")
                q3.metric("Mean", f"{_quality_result.get('mean', float('nan')):.3f}")
                q4.metric("σ", f"{_quality_result.get('sigma', float('nan')):.3f}")
                st.json(_quality_result)
        with tabs[1]:
