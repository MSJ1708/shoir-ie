...
from shoir_industrial_decision_os import render_final_integration, build_copilot_platform_context
...
def _render_post_module_layers(module_name: str) -> None:
    """Run the single final cross-cutting integration surface after native module execution."""
    try:
        render_final_integration(
            str(module_name),
            str(tier_val),
            str(st.session_state.get("current_user", "unknown")),
        )
    except Exception as exc:
        st.warning("Final integration layer encountered a recoverable issue; native module results remain available.")
        with st.expander("Final integration diagnostic", expanded=False):
...
# =====================================================================
# FINAL INDUSTRIAL DECISION OS INTEGRATION
# ---------------------------------------------------------------------
# One post-module call owns canonical Digital Thread synchronization,
# enterprise governance, graph fallback/coverage, connector verification,
# decision-to-value evidence, benchmark evidence and persistence.
# =====================================================================
if (
    st.session_state.get("authenticated")
    and st.session_state.get("current_user")
